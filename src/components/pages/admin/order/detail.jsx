...
const OrderDetail = () => {
  ...
  const normalizeOrder = (payload) => {
    const source =
      payload?.data && typeof payload.data === "object"
        ...
        : payload;
    const nestedOrder =
      source?.order && typeof source.order === "object" ? source.order : source;
    const rawItems =
      nestedOrder?.items ||
    ...
    const shippingFee = toNumeric(
      nestedOrder?.shipping_fee || nestedOrder?.shipping || 0,
    );
    const discount = toNumeric(
      nestedOrder?.discount || nestedOrder?.discount_amount || 0,
    );
    const totalAmount = toNumeric(
      nestedOrder?.total_amount || subtotal + shippingFee - discount,
    );

    return {
      id: nestedOrder?.id ?? source?.id ?? payload?.id ?? "",
      code: displayOrderCode(
        nestedOrder?.id ?? source?.id ?? payload?.id ?? "",
      ),
      ...
      userAddress:
        nestedOrder?.user_address ||
        nestedOrder?.customer_address ||
        nestedOrder?.address ||
        nestedOrder?.user?.address ||
        nestedOrder?.customer?.address ||
        "-",
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      ...
      showToast(
        err.message || "Không thể cập nhật trạng thái đơn hàng",
        "error",
      );
    } finally {
      setIsSaving(false);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase transition-all border ${getStatusStyle(displayedStatus)}`}
            >
              {displayedStatus}
            </span>
          </div>
            ...
            <div className="bg-white rounded-[24px] p-8 shadow-soft border border-gray-50 space-y-6">
              ...
              {!canEditStatus && (
                <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 text-sm text-gray-500">
                  Đơn hàng đã hủy hoặc đã hoàn thành nên không thể sửa trạng
                  thái.
                </div>
              )}
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    ...
                    {order.items.map((item) => (
                      <tr key={item.id} className="group">
                        ...
                        <td className="p-4 pr-0 text-right font-extrabold text-primary">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[24px] p-8 shadow-soft border border-gray-50 space-y-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] bg-green-50 text-green-600 font-bold px-3 py-1 rounded-full border border-green-100 uppercase">
                  {order.paymentMethod}
                </span>
              </div>
              <div className="space-y-4">
