...
const CATEGORY_FILTERS = [
    ...
    { id: "chair-sofa", label: "Ghế & Sofa" },
    { id: "bed", label: "Giường ngủ" },
    { id: "lamp", label: "Đèn trang trí" },
    { id: "table", label: "Bàn làm việc" },
];

const PRODUCTS = [
    {
        id: "p1",
        name: "Ghế Bành Sakarias",
        brand: "IKEA",
        categoryId: "chair-sofa",
        categoryLabel: "Ghế",
        rating: "★★★★★",
        price: "3.500.000đ",
    ...
    {
        id: "p4",
        name: "Giường Ngủ Minimal",
        brand: "Nội thất Xinh",
        categoryId: "bed",
        categoryLabel: "Giường ngủ",
        rating: "★★★★★",
        price: "9.900.000đ",
                        ...
                        <div className="space-y-2">
                            {CATEGORY_FILTERS.map((category) => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between rounded-xl px-4 py-3 text-left transition border ${activeCategory === category.id
                                        ? "bg-orange-50 border-orange-200 text-orange-600"
                                        : "bg-white border-gray-100 text-textMuted hover:text-primary hover:border-gray-200"
                                        }`}
                                >
                ...
                <main className="w-full md:w-3/4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-8 pb-4 border-b border-gray-200 gap-4">
                        <p className="text-sm text-textMuted">
                            Đang hiển thị {filteredProducts.length === 0 ? 0 : 1}-{filteredProducts.length} trong số {filteredProducts.length} sản phẩm
                        </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredProducts.map((product) => (
                            <div key={product.id} className="bg-white rounded-3xl p-4 shadow-sm hover:shadow-xl transition flex flex-col group border border-gray-100 relative">
                                ...
                                <div className="text-xs text-textMuted mb-1 uppercase tracking-wider font-bold italic">{product.brand} / {product.categoryLabel}</div>
                                <div className="text-lg font-semibold text-primary mb-1">{product.name}</div>
                                <div className="flex items-center space-x-1 mb-3 text-xs text-yellow-400">{product.rating}</div>
                    ...
                    {filteredProducts.length === 0 && (
                        <div className="mt-10 rounded-3xl border border-dashed border-gray-200 bg-white p-10 text-center text-sm text-textMuted">
